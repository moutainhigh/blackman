...
public class WXServiceController extends BaseController{
	...
	public ActionResult lookService(String orderid){
		...
		return view("/wx/service/lookservice");
	}
...
//		}
		...
		if(StringUtils.isNotBlank(openId)){
			BFLoginEntity u = WeiXinBuz.wb.getBFLoginEByopenId(openId);
			if(u!=null){
				userid = u.getUserid();//38412528704513 || asdnasdjsajjdjsajdjklasjdasjkdjsadj
			}
		}
		...
		long count = 0;
		if(userid!=0){
			List<VendorServeEntity> pageVendorServList = RSBLL.getstance().getVendorServeService().getVendorServeListBycondition(" userid ="+userid+" and (status = 9 or status = 10) ", 1, 99, "  starttime desc ");
	...
	public ActionResult free(){
			...
			try{
				String openId  = model().get("openId").toString();
				// openId = "oxizHs97Dv0Kb7L29D782LkdMzE4";
				if(StringUtils.isNotBlank(openId)){
			...
			//	variable.put("prodcateid", "3000");
				...
				if(pageHisProcList != null && !pageHisProcList.isEmpty()){
					for(LvProcInstEntitiy pageHisProc:pageHisProcList){
						...
						List<LvHiActEntity> taskList = RSBLL.getstance().getWfReceiveTaskService().getListReceiveTaskByProcessInstanceId(pageHisProc.getId());
						if(CollectionUtils.isNotEmpty(taskList)){
							processVariables.put("taskId", taskList.get(taskList.size()-1).getId());
						}
						...
						LvEnterpriseEntity enterpriseEntity = null;
						try {
							
							enterpriseEntity = RSBLL.getstance().getEpEnterpriseService().getEnterpriseById(Long.parseLong(busidObj.toString()));
						...
						if(enterpriseEntity!=null){
							...
							processVariables.put("enterpriseName", StringUtils.isEmpty(enterpriseEntity.getName())?"":enterpriseEntity.getName());
							long cityid = enterpriseEntity.getCityId();
							long areaid = enterpriseEntity.getAreaId();
							
							//中文城市+区域
							BFAreasEntity a1 = RSBLL.getstance().getAreaService().getAeasEntityById(areaid);
							if(a1!=null){
								String areaname = a1.getName();
								processVariables.put("areaname", areaname);
							}
							BFAreasEntity a2 = RSBLL.getstance().getAreaService().getAeasEntityById(cityid);
							if(a2!=null){
								String cityname = a2.getName();
								processVariables.put("cityname", cityname);
							}
				...
				model.add("list", pageHisProcList==null?"":pageHisProcList);
				}
