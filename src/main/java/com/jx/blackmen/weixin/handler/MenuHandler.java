package com.jx.blackmen.weixin.handler;
...
//	static String center_yijian = "";
	...
	public static String getUrl(String targetUrl){
		String tempUrl = "";
		try {
			tempUrl = MContents.WX_url_base_shouquan_page.replace("APP_ID", MContents.weixin_app_id).replace("REDIRECT_URI", URLEncoder.encode(MContents.weixin_hosts+targetUrl, "utf-8"));
	...
	static{
		menustr = "{\"button\":["
				...
				+"{\"name\":\"免费工具\",\"sub_button\":["
					...
					+ "{\"type\":\"view\",\"name\":\"小微资讯\",\"url\":\""+getUrl(zixun)+"\"}"
					+ "]},";
				/*
	...
	public String creatMenu(){
		...
		IWeixinService ws = RSBLL.getstance().getWeixinService();
		String token = "";
		try {
			token = ws.getWeixinToken(MContents.weixin_app_id, MContents.weixin_app_secret_id);
		} catch (Exception e1) {
